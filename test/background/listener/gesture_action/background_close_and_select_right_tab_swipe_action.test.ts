...
describe('BackgroundCloseAndSelectRightTabGestureActionクラスのテスト', () => {
    ...
    let action: BackgroundCloseAndSelectRightTabGestureAction;
    ...
    beforeEach(() => {
        ...
        container.bind(BackgroundCloseAndSelectRightTabGestureAction).toSelf();
        action = container.get(BackgroundCloseAndSelectRightTabGestureAction);
    });
    ...
    describe('doActionメソッドのテスト', () => {
        it('タブIDが存在する場合、右のタブを選択して現在のタブを閉じる', async () => {
            const sender = {
                tab: { id: 123 }
            };
            ...
            expect(mockChromeTabOperator.activateRightTabAndCloseCurrentTab).toHaveBeenCalledWith(123);
            expect(mockLogger.debug).toHaveBeenCalledWith('バックグラウンドで「このタブを閉じて右のタブを選択」のジェスチャーを実行');
        });
        ...
        it('タブIDが存在しない場合、何もしない', async () => {
            ...
            expect(mockChromeTabOperator.activateRightTabAndCloseCurrentTab).not.toHaveBeenCalled();
