...
describe('BackgroundSelectLeftTabGestureActionクラスのテスト', () => {
    ...
    let action: BackgroundSelectLeftTabGestureAction;
    ...
    beforeEach(() => {
        ...
        container.bind(BackgroundSelectLeftTabGestureAction).toSelf();
        action = container.get(BackgroundSelectLeftTabGestureAction);
    });
    ...
    describe('doActionメソッドのテスト', () => {
        it('タブIDが存在する場合、左のタブを選択する', async () => {
            const sender = {
                tab: { id: 123 }
            };
            ...
            expect(mockChromeTabOperator.activateLeftTab).toHaveBeenCalledWith(123);
            expect(mockLogger.debug).toHaveBeenCalledWith('バックグラウンドで「左のタブを選択」のジェスチャーを実行');
        });
        ...
        it('タブIDが存在しない場合、何もしない', async () => {
            ...
            expect(mockChromeTabOperator.activateLeftTab).not.toHaveBeenCalled();
