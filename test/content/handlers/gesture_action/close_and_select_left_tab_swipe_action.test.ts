...
describe('CloseAndSelectLeftTabGestureActionクラスのテスト', () => {
    let action: CloseAndSelectLeftTabGestureAction;
    ...
    beforeEach(() => {
        ...
        // テスト対象のインスタンスを取得
        action = container.get(CloseAndSelectLeftTabGestureAction);
    });
    ...
    describe('doActionメソッドのテスト', () => {
        it('メッセージが正しく送信されること', async () => {
            ...
            expect(mockMessenger.sendMessage).toHaveBeenCalledWith(BackgroundMessage.CloseAndSelectLeftTab);
        });
        ...
        it('デバッグログが出力されること', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith('「このタブを閉じて左のタブを選択」のジェスチャーを実行');
        });
