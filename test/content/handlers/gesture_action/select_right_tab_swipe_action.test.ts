...
describe('SelectRightTabGestureActionクラスのテスト', () => {
    let action: SelectRightTabGestureAction;
    ...
    beforeEach(() => {
        ...
        // テスト対象のインスタンスを取得
        action = container.get(SelectRightTabGestureAction);
    });
    ...
    describe('doActionメソッドのテスト', () => {
        it('メッセージが正しく送信されること', async () => {
            ...
            expect(mockMessenger.sendMessage).toHaveBeenCalledWith(BackgroundMessage.SelectRightTab);
        });
        ...
        it('デバッグログが出力されること', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith('「右のタブを選択」のジェスチャーを実行');
        });
