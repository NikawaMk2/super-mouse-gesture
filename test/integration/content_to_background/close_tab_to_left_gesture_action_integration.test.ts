...
describe('CloseTabToLeftGestureAction Integration', () => {
    ...
    let closeTabToLeftGestureAction: CloseTabToLeftGestureAction;
    ...
    beforeEach(() => {
        ...
        // テスト対象のアクション初期化
        closeTabToLeftGestureAction = new CloseTabToLeftGestureAction(messageSender);
    ...
    it('コンテンツスクリプトからバックグラウンドスクリプトへ左側のタブを閉じるジェスチャアクションを実行できること', async () => {
        // アクションの実行
        await closeTabToLeftGestureAction.execute();

        // chrome.tabs.removeがアクティブタブに対して呼ばれたことを確認
        expect(chrome.tabs.remove).toHaveBeenCalledWith(3, expect.any(Function));
        // chrome.tabs.updateが左隣のタブに対して呼ばれたことを確認
    ...
    it('アクティブなタブが最初のタブの場合、chrome.tabs.removeのみが呼ばれ、エラーも発生しないこと', async () => {
        // アクティブなタブが最初のタブの状態をモック
        (chrome.tabs.query as jest.Mock).mockImplementation((queryInfo, callback) => {
            if (queryInfo.active) {
