...
describe('OpenInBackgroundTabDragAction Integration', () => {
    ...
    it('content→background経由でOpenInBackgroundTabDragActionが呼ばれ、最終的にchrome.tabs.createがactive: falseで実行されること', async () => {
        ...
        expect(chrome.tabs.create).toHaveBeenCalledWith({ url: 'https://example.com/', active: false }, expect.any(Function));
    });
}); 