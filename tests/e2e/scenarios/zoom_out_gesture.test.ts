...
test.describe('ページ縮小ジェスチャー', () => {
  ...
  test('右クリックしながら左→下方向にマウスを動かすと、ページが縮小すること', async () => {
    // テスト用のHTMLページを作成
    const htmlContent = `
      <!DOCTYPE html>
      <html>
    ...
    try {
      ...
      // 右クリックを押しながら左→下方向にマウスを動かす（LDジェスチャー）
      // 中央付近の位置で開始
      ...
      const actionName = await page.evaluate(() => {
        ...
        const actionNameDiv = divs.find((div) => {
          ...
          return (
            ...
            div.textContent?.trim() === 'ページ縮小'
          );
        });
        return actionNameDiv?.textContent?.trim() ?? null;
      });
      expect(actionName).toBe('ページ縮小');
