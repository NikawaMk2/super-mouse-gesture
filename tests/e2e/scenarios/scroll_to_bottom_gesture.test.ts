...
/**
 * ページ最下部へスクロールジェスチャーのE2Eテスト
 */
test.describe('ページ最下部へスクロールジェスチャー', () => {
  ...
  test('右クリックしながら上方向、その後下方向にマウスを動かすと、ページが最下部へスクロールすること', async () => {
    // スクロール可能なHTMLページを作成
    const htmlContent = `
      <!DOCTYPE html>
      <html>
    ...
    try {
      ...
      // ページを下にスクロール（テストの前提条件として、上にスクロールできる状態にする）
      // ただし、最下部にはしない（最下部にスクロールすることをテストするため）
      ...
      // スクロール後の位置を確認（下にスクロールされているが、最下部ではないことを確認）
      const scrollYBeforeGesture = await page.evaluate(() => window.scrollY);
      expect(scrollYBeforeGesture).toBeGreaterThan(0);
      expect(scrollYBeforeGesture).toBeLessThan(scrollHeight - viewportHeight);

      // 右クリックを押しながら上方向、その後下方向にマウスを動かす（UDジェスチャー）
      // 中央付近の位置で開始
      ...
      // 最終位置はstartYより少し下に留める（距離チェックに引っかからないようにするため）
      for (let i = 1; i <= steps; i++) {
      ...
      const actionName = await page.evaluate(() => {
        ...
        const actionNameDiv = divs.find((div) => {
          ...
          return (
            ...
            div.textContent?.trim() === 'ページボトムへ'
          );
        });
        return actionNameDiv?.textContent?.trim() ?? null;
      });
      expect(actionName).toBe('ページボトムへ');
      ...
      // スクロールが完了するまで待機
      // scrollTo({ top: scrollHeight, behavior: 'auto' }) は即座にスクロールするが、念のため少し待機
      await new Promise((resolve) => setTimeout(resolve, 500));

      // スクロール後の位置を確認（最下部（scrollY ≈ scrollHeight - viewportHeight）までスクロールされていることを確認）
      const scrollYAfterGesture = await page.evaluate(() => window.scrollY);
      const maxScrollY = scrollHeight - viewportHeight;
      // スクロール位置が最下部に近いことを確認（数ピクセルの誤差を許容）
      expect(scrollYAfterGesture).toBeGreaterThanOrEqual(maxScrollY - 10);
      expect(scrollYAfterGesture).toBeLessThanOrEqual(maxScrollY + 10);
    } finally {
      // サーバーを閉じる
