...
/**
 * タブを閉じて左へジェスチャーのE2Eテスト
 */
test.describe('タブを閉じて左へジェスチャー', () => {
  ...
  test('右クリックしながら下方向、次に左方向にマウスを動かすと、現在のタブが閉じられて左のタブに移動すること', async () => {
    // タブ1のHTMLコンテンツ
    const tab1Content = `
      <!DOCTYPE html>
      <html>
    ...
    try {
      ...
      // 右クリックを押しながら下方向、次に左方向にマウスを動かす（DLジェスチャー）
      // 中央付近の位置で開始
      ...
      const actionName = await page2.evaluate(() => {
        ...
        const actionNameDiv = divs.find((div) => {
          ...
          return (
            ...
            div.textContent?.trim() === 'タブを閉じて左へ'
          );
        });
        return actionNameDiv?.textContent?.trim() ?? null;
      });
      expect(actionName).toBe('タブを閉じて左へ');
      ...
      // タブ1がアクティブになったことを確認（これにより、タブ2が閉じられたことが間接的に確認される）
      ...
      // タブ3もまだ存在することを確認（タブ2だけが閉じられた）
      await page3.bringToFront();
      await new Promise((resolve) => setTimeout(resolve, 500));
      const tab3Title = await page3.title();
      expect(tab3Title).toBe('Tab 3');
    } finally {
      // サーバーを閉じる
