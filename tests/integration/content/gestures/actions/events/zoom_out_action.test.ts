/**
 * zoom_out_action.ts のインテグレーションテスト
 * 
...
describe('zoomOutAction', () => {
  ...
  it('コンテンツスクリプトからバックグラウンドへのメッセージフローが正常に動作すること_ページが縮小されること', async () => {
    ...
    expect(mockSendMessage).toHaveBeenCalledWith({
      type: ExtensionMessageType.ZOOM_OUT,
      payload: null,
    });
    ...
    expect(mockTabsSetZoom).toHaveBeenCalledWith(1, 0.9); // 1.0 - 0.1 = 0.9
    
    // レスポンスが送信されたことを確認
    expect(sendResponse).toHaveBeenCalledWith({ success: true });
  });
  
  it('現在のズームレベルが最小値の場合_ページが縮小されないこと', async () => {
    // バックグラウンド側のメッセージハンドラーを設定
    setupMessageHandlers();
    ...
    mockTabsQuery.mockResolvedValue(mockTabs);
    mockTabsGetZoom.mockResolvedValue(0.25); // 現在のズームレベルは最小値0.25
    ...
    expect(mockTabsQuery).toHaveBeenCalledWith({ active: true, currentWindow: true });
    expect(mockTabsGetZoom).toHaveBeenCalledWith(1);
    // 最小値0.25なので、0.25 - 0.1 = 0.15ではなく、Math.max(0.15, 0.25) = 0.25が設定される
    expect(mockTabsSetZoom).toHaveBeenCalledWith(1, 0.25);
    
    // レスポンスが送信されたことを確認
    expect(sendResponse).toHaveBeenCalledWith({ success: true });
  });
  
  it('アクティブなタブが存在しない場合_ページが縮小されないこと', async () => {
    // バックグラウンド側のメッセージハンドラーを設定
    setupMessageHandlers();
  ...
  it('タブIDが存在しない場合_ページが縮小されないこと', async () => {
    // バックグラウンド側のメッセージハンドラーを設定
    setupMessageHandlers();
