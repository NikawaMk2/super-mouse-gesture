...
@injectable()
export default class BackgroundSelectRightTabGestureAction implements BackgroundGestureAction {
    constructor(
    ...
    async doAction(sender: chrome.runtime.MessageSender): Promise<void> {
        ...
        Logger.debug('バックグラウンドで「右のタブを選択」のジェスチャーを実行');
        await this.chromeTabOperator.activateRightTab(sender.tab.id);
    }
}